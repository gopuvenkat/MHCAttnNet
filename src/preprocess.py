...
class Preprocess:
    def __init__(self, file_name, header_index, relevant_class, relevant_orgs, is_quan):
        ...
        # load the .csv file
        self.data = pd.read_csv(file_name, header=header_index)
        ...
        self.data = self.data[self.data["assay_group"].isin(["dissociation constant KD (~IC50)", "half maximal inhibitory concentration (IC50)"])]
        ...
        self.data['valid'] = self.data['peptide'].apply(lambda x: x.isupper())
...
if __name__ == '__main__':
    ...
    parser.add_argument('--is_quan', default=True)
